...
void Utility::generateProblem(unsigned int seed, ProblemData &problemData) {
    ...
    // Set the pseudo random number伪随机 generator seed used for generating encryption keys密码健
    srand(seed);

    writeMap(seed, problemData);
    writeInitialStormData(seed, problemData);

    // Ensure that the destination目的地 doesn't go out of bounds越界 and that the ship doesn't start on land
    // 船出发的位置
    do {
