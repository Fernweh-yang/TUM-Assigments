...
void simulate_waves(ProblemData &problemData) {
    ...
    for (int x = 1; x < MAP_SIZE - 1; ++x) {
        for (int y = 1; y < MAP_SIZE - 1; ++y) {
            ...
            // The acceleration is multiplied with an attack value, specifying明确规定 how fast the system can accelerate.
            ...
            // energy preserved保留的 takes into account that storms lose energy to their environments over time. The
            // ratio比例 of energy preserved is higher on open water, lower close to the shore暗 and 0 on land.
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    for (int problem = 0; problem < numProblems; ++problem) {
        // ProblemData定义在a-star-navigator.h里
        ...
        int pathLength = -1;
        // Position2D类定义在a-star-navigator.h里。
        std::vector<Position2D> path;

        // TIME_STEPS定义在a-star-navigator.h里，在600步内找到路径
